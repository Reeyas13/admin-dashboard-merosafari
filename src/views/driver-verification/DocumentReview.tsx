...
const DocumentReview: React.FC = () => {
  ...
  const [rejectionReason, setRejectionReason] = useState(''); // Added
  ...
  const handleSubmitReview = async () => {
    ...
    try {
      ...
      // Add rejection_reason only when rejecting
      if (reviewStatus === 'rejected') {
  ...
  const getDocumentIcon = (documentType: string) => {
    const icons: Record<string, React.ReactNode> = {
      drivers_license: <FileText className="w-5 h-5" />,
      vehicle_registration: <File className="w-5 h-5" />,
      vehicle_insurance: <Shield className="w-5 h-5" />,
      vehicle_photo: <Camera className="w-5 h-5" />,
      profile_photo: <User className="w-5 h-5" />,
      background_check: <FileText className="w-5 h-5" />,
    };
