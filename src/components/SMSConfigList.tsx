...
import { Plus, Edit, Trash2, CheckCircle, MessageSquare } from 'lucide-react';

interface SMSConfigListProps {
  onEdit?: (config: SMSConfig) => void;
  onAdd?: () => void;
}

export const SMSConfigList: React.FC<SMSConfigListProps> = ({ onEdit, onAdd }) => {
  ...
  const fetchConfigs = async () => {
    try {
      setLoading(true);
      setError(null);
      const response = await smsConfigService.getAllConfigs();
      if(Array.isArray(response.configs)){
        
        setConfigs(response.configs);
      }
    } catch (err: any) {
      setError(err.message || 'Failed to fetch SMS configurations');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleActivate = async (id: number) => {
    try {
      await smsConfigService.activateConfig(id);
      fetchConfigs();
    } catch (err: any) {
  ...
  const getProviderBadgeVariant = (provider: string) => {
    const variants: Record<string, any> = {
      test: 'lightWarning',
      smspasal: 'lightPrimary',
      samayasms: 'lightSuccess',
      sparrow: 'lightInfo',
    };
    ...
    return (
      <div className="flex items-center justify-center py-12">
        <div className="text-center">
          ...
          <p className="mt-4 text-muted-foreground">Loading SMS configurations...</p>
        </div>
      </div>
    );
  }
    ...
    <div className="space-y-6">
      ...
      <Breadcrumb>
        <BreadcrumbList>
          ...
          <BreadcrumbSeparator />
          <BreadcrumbItem>
            <BreadcrumbPage>SMS Configuration</BreadcrumbPage>
          </BreadcrumbItem>
        </BreadcrumbList>
      </Breadcrumb>
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold text-foreground">SMS Configurations</h1>
          <p className="text-muted-foreground mt-1">
            Manage SMS service providers for notifications
          </p>
        </div>
        {onAdd && (
              ...
              <div className="space-y-2 mb-4">
                ...
                {config.sender_id && (
                  <div className="flex justify-between text-sm">
                    ...
                    <span className="font-medium">{config.sender_id}</span>
                  </div>
                )}
                {config.api_url && (
                  <div className="flex justify-between text-sm">
      ...
      {configs.length === 0 && (
        <div className="text-center py-12 rounded-lg border bg-card">
          ...
          <p className="text-muted-foreground mb-6">
            Add your first SMS provider to start sending notifications
          </p>
          {onAdd && (
            <Button onClick={onAdd}>
              <Plus className="mr-2 h-4 w-4" />
